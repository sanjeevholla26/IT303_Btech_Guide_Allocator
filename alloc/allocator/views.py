...
def register(request):
    if request.method == "POST":
        ...
        try:
            user = MyUser.objects.create_user(eduMailID=eduMailID, email=email, username=username)
            user.save()
...
def login_view(request) :
    if not request.user.is_authenticated:
        if request.method == "POST" :
            eduMailID = request.POST["eduMailID"]
            next = request.POST["next"]
            user = MyUser.objects.get(eduMailID=eduMailID)
